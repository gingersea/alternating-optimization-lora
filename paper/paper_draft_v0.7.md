...
2. **7B-scale validation of the 2×2 matrix (3/4 cells).** Protocol B (AdamW+full-rank, 7B trainable parameters) achieves PPL 1.25 ± 0.01 (N=3, N_EVAL=200 protocol) on Qwen2.5-7B at 800 steps — an 8.3× improvement over Protocol D (LoRA r=8, ~3M trainable parameters) and an ~84× improvement over the untrained model (N_EVAL=200 baseline PPL=105.56). Protocol A is blocked by the depth boundary — confirmed via 11 attempts across two distributed backends. The 7B results quantify the performance impact of scaling trainable parameters from 3M to 7B under identical optimizer and training conditions; the 2300× parameter ratio means that parameter count, rather than parameter form *per se*, is the most parsimonious explanation for the observed 8.3× gap. We include a parameter-matched ablation (§5.7) to further isolate form effects. A full-test-set evaluation was also performed on Protocol B only (298,938 tokens, baseline PPL=133.16, B PPL=1.25); these results are consistent with N_EVAL=200 within ±0.01 PPL.
...
We resolve this through a quasi-factorial 2×2 design:
...
†Protocol C uses SGD+Perturbation only (no ALS — quasi-factorial, §3.2). *GPT-2 Protocol D uses built-in LoRALayer (Conv1D-compatible). ‖7B results at 800 steps with N=3 seeds, N_EVAL=200 protocol (~12,640 tokens). Baseline PPL=105.56. Full-test-set evaluation on B only: 298,938 tokens, consistent within ±0.01 PPL. Protocol A blocked — see §5.6.

**Multi-Seed Replication (OPT-125m, 200 steps, N=3):**
Protocol D achieves perplexity $16.0 \pm 0.5$ (CV=3.2%), Protocol B achieves $18.7 \pm 0.4$ (CV=2.3%), Protocol A achieves $1,373 \pm 558$ (CV=40.6%). On Qwen2.5-7B at 800 steps (N=3, N_EVAL=200), Protocol B achieves PPL 1.25 ± 0.01 (CV<1%), Protocol D achieves 10.41 ± 0.01, and Protocol C achieves 135.36 ± 9.1. The high CV of Protocol A is consistent across architectures and step counts (see Section 5.3).
...
The fresh (untrained) Qwen2.5-7B baseline is PPL=133.16 on the full WikiText-2 test set (298,938 tokens) and PPL=105.56 under the N_EVAL=200 protocol (~12,640 tokens). Protocol B's full-test-set evaluation (PPL=1.25, 106× over the full-test-set baseline) confirms effective full-rank fine-tuning on the training domain. However, cross-dataset evaluation on C4 (§5.6.4) reveals that the 8.3× WikiText-2 gap between B and D collapses to 1.1× on web text (C4 PPL: B=2.66, D=2.43), and HellaSwag accuracy *decreases* after full-rank training (§5.6.3). The near-perfect WikiText-2 PPL (1.25) therefore reflects in-distribution overfitting rather than improved language modeling. A parameter-matched ablation (§5.7) further demonstrates that increasing LoRA rank from 8 to 256 improves PPL by 20× — the apparent full-rank advantage at low rank is a parameter-count artifact, not a form effect. The cross-seed CV<1% confirms training stability.
...
**Figure 1**: Quasi-factorial 2×2 design schematic. Left panel: the attribution problem (confounded comparison). Right panel: four protocols resolving the confound.
